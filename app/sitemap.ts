...
import { redirectionFiche } from "@/lib/notaires-source";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Seules les fiches de notaires INSCRITS restent ici. Les ~19 600 fiches de
  // l'annuaire importé sont déclarées dans /annuaire-sitemap.xml.
  //
  // Elles y étaient toutes, et ce sitemap atteignait 20 281 URL. Résultat
  // mesuré le 2026-09-25 : les 285 pages de ville étaient encore « URL is
  // unknown to Google » — jamais visitées, noyées derrière l'annuaire — alors
  // que Google répondait à « notaire la ciotat » avec un article de blog
  // générique, et plaçait quand même le site 2e. Un sitemap qu'un domaine de
  // quatre mois peut explorer en entier vaut mieux qu'un sitemap exhaustif.
  const fichesCompletees = await idsNotairesInscrits();
  const notairePages: MetadataRoute.Sitemap = [...fichesCompletees]
    .filter((id) => !estFicheTest(id) && !redirectionFiche(id))
    .map((id) => ({
      url: `${BASE}/notaires/${id}`,
